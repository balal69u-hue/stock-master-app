...
app = FastAPI(title="TAS Master Live Engine")
...
def track_adobe_stock(query: str, search_type: str = "contributor", sort_by: str = "relevance"):
    ...
    # Adobe Stock Direct Ajax JSON Endpoint
    ...
    # ScraperAPI Direct Request
    scraper_url = f"http://api.scraperapi.com?api_key={SCRAPER_API_KEY}&url={urllib.parse.quote(adobe_target)}&keep_headers=true"
    ...
    try:
        res = requests.get(scraper_url, headers=custom_headers, timeout=30)
