...
def track_adobe_stock(query: str, search_type: str = "contributor", sort_by: str = "relevance"):
    ...
    gateway_url = f"http://api.scraperapi.com?api_key={SCRAPER_API_KEY}&url={urllib.parse.quote(adobe_target)}"

    try:
        res = requests.get(gateway_url, timeout=30)
